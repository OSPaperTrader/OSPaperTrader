...
router.post('/signup', (req, res, next) => {
  ...
  if (validUser(user)) {
    ...
    db.query(`select * from users where email = '${user.email}'`).then(
      result => {
        ...
        if (!result.rowCount) {
          bcrypt.hash(user.password, 5).then(hash => {
            ...
            db.query(
              `insert into users (email, firstname, lastname, password) values ('${user.email}', '${user.firstName}', '${user.lastName}', '${user.password}')`
            ).then(insertResponse => {
              db.query(`select * from users where email ='${user.email}'`).then(
                id => {
                  ...
                  //we can redirect
                  setUserIdCookie(req, res, id.rows[0].id);
